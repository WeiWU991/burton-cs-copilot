...
def normalize_product_id(query):
    ...
    for num in all_numbers:
        base_id = num[:6]
        if len(num) == 6:
            hints.append(f"货号前缀为 {num}")
        else:
            hints.append(f"客户输入长货号 {num}，其核心Base ID为 {base_id}，请忽略颜色码")
    
    # 🔴 关键修复：使用 XML 标签隔离后台逻辑，防止 AI 漏嘴
    hint_text = f"\n\n<system_context>后台检索辅助：{'; '.join(hints)}。</system_context>"
...
with st.sidebar:
    ...
    if st.session_state.banned_words: st.success(f"🛡️ 护盾激活 ({len(st.session_state.banned_words)} 词条)")
...
if app_mode == "💬 客服实战副驾":
    ...
    3. 货号逻辑：如果用户问题中包含 <system_context> 标签，请利用里面的线索（如Base ID前缀）进行精准检索。但【严禁】在你的回复中输出或解释任何关于 Base ID、截取货号或 system_context 的信息！只需直接输出产品推荐。
    4. 合规：严禁极限词。隐藏具体金额。
    输出必须包含：### 1️⃣ 🧠 客户画像分析、### 2️⃣ 📚 核心知识胶囊、### 3️⃣ 💬 建议回复话术、### 4️⃣ 🎯 关联销售机会。
    """

    user_query = st.chat_input("在此输入客户问题 (如：查货号10014109301)...")
    if user_query:
