    ...
    for txt_file in txt_files:
        try:
            with open(txt_file, "r", encoding='utf-8') as f:
                ...
                for w in raw_words:
                    clean_w = w.strip()
                    # 🔴 核心修改：允许单字 "最" 通过筛选，其他单字依然忽略（防止误杀）
...
with st.sidebar:
    ...
    st.caption("🛡️ 合规护盾 (增强版)")
    if st.session_state.banned_words:
        st.success(f"✅ 已激活 ({len(st.session_state.banned_words)} 词条, 含'最')")
    else:
        st.warning("⚠️ 未激活")
...
2. **主动反问协议 (Active Inquiry)**：
   - **严禁猜测**：当用户咨询产品推荐时，如果用户**没有主动提供**【性别】、【体重】、【鞋码】或【滑行水平】，**严禁**直接假设用户是特定性别或体型。
   - **强制执行**：在给出初步推荐（基于现有模糊信息）的同时，回复的**最后一段话必须是反问句**，引导用户提供缺失信息以便精准推荐。
     - *Bad Case*: "为您推荐 Yeasayer..." (错误：没问性别就默认推女板)
     - *Good Case*: "Burton 有很多优秀的选择。为了帮您匹配最合适的硬度和长度，请问您的**性别、体重**和**鞋码**是多少？"

3. **合规第一 (Compliance)**：严禁使用极限词（如：第一、最、最强、顶级）。
   - **"最"字零容忍**：请尽量使用"十分"、"非常"、"出色"来替代"最"字。

4. **价格策略 (Price Hiding)**：严禁在最终回复中写出具体的金额数字。引导"具体价格请以店铺实时活动为准"。

5. **格式严格**：必须严格遵守下面的 Markdown 结构。

# 输出视图结构
---
### 1️⃣ 🧠 客户画像分析
* **客户类型**: 
* **关键缺项**: [⚠️ 高亮显示缺失信息，如：体重/性别/鞋码]
* **情绪指数**: [⭐⭐⭐⭐⭐]
...
### 3️⃣ 💬 建议回复话术
> **请复制以下内容发送给客户：**
> "[建议回复内容。包含初步回答 + **必须反问缺失信息**。语气亲切，严禁极限词。]"
        ...
        try:
            ...
            with st.chat_message("assistant", avatar="🏂"):
                with st.spinner("🤖 YZ-Shield 正在检索企业知识库..."):
                    ...
                    if has_issues:
                        st.toast("🛡️ 已替换极限词(含'最')，价格已按策略隐藏。", icon="✅")
